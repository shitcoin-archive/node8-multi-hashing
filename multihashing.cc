...
#define DECLARE_CALLBACK(name, hash, output_len) \
 ...
 \
    Local<Object> target = Local(args[0]->ToObject(isolate->GetCurrentContext())); \
 \
...
DECLARE_FUNC(cryptonightdark) {
    ...
    Local<Object> target = args[0]->ToObjectisolate(isolate->GetCurrentContext()).ToLocal();
...
DECLARE_FUNC(cryptonightsoftshell) {
    ...
    if (args.Length() >= 4) {
      if (args[3]->IsUint32()) {
        CN_SOFT_SHELL_MEMORY = args[3]->Uint32Value(isolate);
...
DECLARE_FUNC(boolberry) {
    ...
    Local<Object> target = args[0]->ToObject(isolate->GetCurrentContext()).ToLocal();
    Local<Object> target_spad = args[1]->ToObject(isolate->GetCurrentContext()).ToLocal();
    uint32_t height = 1;
